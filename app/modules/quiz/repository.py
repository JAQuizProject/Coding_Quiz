...
설계 주의사항:
- 이 레포지토리는 낮은 수준의 DB 작업(쿼리 실행)을 담당합니다. 비즈니스 로직(점수 계산, 결과 메시지 등)은
  서비스 계층에서 처리하는 것을 권장합니다.
- 현재 `upsert_score`는 내부에서 `commit()`을 호출합니다. 복수의 DB 작업을 하나의 트랜잭션으로 묶어야
  하는 경우(서비스 레이어에서 관리)에는 commit 위치를 서비스로 옮기는 것이 좋습니다.
"""
...
class QuizRepository:
    ...
    def fetch_quizzes(self, category: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        Args:
            category (Optional[str]): 카테고리 필터. None이면 전체 퀴즈를 반환합니다.

        Returns:
            List[Dict[str, Any]]: 각 퀴즈를 딕셔너리 형태로 담은 리스트. 키: id, question, explanation, answer
        """
        if category:
    ...
    def fetch_categories(self) -> List[str]:
        ...
        Returns:
            List[str]: 중복 제거된 카테고리 문자열 목록. NULL/빈 값은 제외됩니다.
        """
    ...
    def upsert_score(self, user_id: int, category: str, score_percentage: float):
        ...
        주의:
        - 이 메서드는 내부에서 `commit()`을 수행합니다. 여러 DB 조작을 하나의 트랜잭션으로 묶어야
          할 경우(예: 여러 레포지토리 호출)에는 서비스 레이어에서 트랜잭션을 관리하도록 변경하는 것이 안전합니다.
