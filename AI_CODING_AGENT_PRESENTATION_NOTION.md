...
> 라이브 발표에서는 `FULLSTACK_AI_DEBUGGING_DEMO_RUNBOOK.md`의 Chrome MCP 재현 절차를 먼저 실행한 뒤, 이 문서의 실패 테스트·최소 수정·검증 흐름으로 이어간다. 이 문서의 명령행 재현은 브라우저 연결 실패 시 사용하는 백업이다.
...
- MCP: 실제 브라우저 재현과 Network 근거 수집에 사용하며, 코드의 정답 기준은 테스트로 고정
