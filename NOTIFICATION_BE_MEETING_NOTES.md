...
### 4. 공용 PC/계정 전환 시 token 소유권이 이전 유저에 남음

확인된 현상:

```text
같은 브라우저에서 seed_user_001로 token 등록
-> NotificationDevice_TM.UserId = seed_user_001

같은 브라우저에서 seed_user_002로 다시 로그인
-> Firebase getToken()이 같은 token 반환 가능
-> /v1/devices 등록 시 Device already exists
-> seed_user_002에는 device token이 등록되지 않음
-> seed_user_002로 sendUser 호출 시 target_count=0 가능
```

왜 문제인지:

FCM token은 사용자 계정이 아니라 브라우저/기기 주소에 가깝다.
PC방, 공용 PC, 다중 계정 테스트처럼 같은 브라우저에서 계정을 바꾸면 같은 token이 다른 로그인 사용자에게 연결되어야 할 수 있다.

현재처럼 token 중복을 무조건 400으로 막으면 새 로그인 사용자에게 알림을 보낼 device가 생기지 않는다.
반대로 기존 사용자에게 token 소유권이 남아 있으면 로그아웃 후에도 공용 브라우저로 기존 사용자 알림이 갈 수 있어 보안/개인정보 측면에서도 위험하다.

대책:

```text
같은 token이 이미 있으면 에러로 보지 않는다.
현재 로그인 UserId로 token 소유권을 갱신한다.
DeviceFamily, BrowserFamily, LastUsedAt, State를 함께 갱신한다.
기존 UserId가 다른 경우 기존 row를 현재 UserId로 update하거나 이전 row를 비활성화 후 새 row를 만든다.
로그아웃 시 가능하면 현재 token을 unregister 또는 State=0 처리한다.
```

정리:

```text
알림 발송은 UserId 기준이 맞다.
다만 FCM token은 현재 로그인 사용자의 기기 주소로 계속 최신화되어야 한다.
/v1/devices는 단순 insert API가 아니라 idempotent upsert 성격이어야 한다.
```

### 5. Firebase Admin SDK 설정 누락 시 sendUser가 500으로 터짐
